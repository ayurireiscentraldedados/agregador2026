const GovernadorApp = {
    ...
    popularSelectEstado() {
        const selectEstado = document.getElementById('select-estado-gov');
        const estados = DataService.getEstadosGovernador();
    ...
    popularSelectTurno() {
        const selectEstado = document.getElementById('select-estado-gov');
        const selectInstituto = document.getElementById('select-instituto-gov');
        const selectTurno = document.getElementById('select-turno-gov');

        const estadoSelecionado = selectEstado.value;
        const institutoSelecionado = selectInstituto ? selectInstituto.value : null;

        const turnos = DataService.getTurnosGovernador(estadoSelecionado, institutoSelecionado);
    ...
    popularSelectCenario() {
        ...
        const selectTurno = document.getElementById('select-turno-gov');
        const selectInstituto = document.getElementById('select-instituto-gov');
        const selectCenario = document.getElementById('select-cenario-gov');

        const estadoSelecionado = selectEstado.value;
        const turnoSelecionado = selectTurno ? selectTurno.value : null;
        const institutoSelecionado = selectInstituto ? selectInstituto.value : null;

        let cenarios = DataService.getCenariosGovernador(estadoSelecionado, turnoSelecionado, institutoSelecionado);
    ...
    popularSelectInstituto() {
        ...
        const institutos = DataService.getInstitutosGovernador(estadoSelecionado);
    ...
    configurarEventos() {
        document.getElementById('select-estado-gov').addEventListener('change', () => {
            this.ocultos.clear();
            this.popularSelectInstituto();
            this.popularSelectTurno(); // que por sua vez chama popularSelectCenario()
            this.atualizarGrafico();
        });
        document.getElementById('select-turno-gov').addEventListener('change', () => {
            this.ocultos.clear();
            this.popularSelectCenario();
            this.atualizarGrafico();
        });
        document.getElementById('select-cenario-gov').addEventListener('change', () => this.atualizarGrafico());
        document.getElementById('select-instituto-gov').addEventListener('change', () => {
            this.ocultos.clear();
    ...
    atualizarGrafico() {
        const estado = document.getElementById('select-estado-gov').value;
        const turno = document.getElementById('select-turno-gov').value;
        const cenario = document.getElementById('select-cenario-gov').value;
        const instituto = document.getElementById('select-instituto-gov').value;

        if (!estado || !turno || !cenario) return;

        const seriesData = DataService.getDadosGovernador(estado, turno, cenario, instituto);
    ...
    renderizarLegenda(legendItems, legendColors) {
        const container = document.getElementById('legenda-governador');
        if (!container) return;
        container.innerHTML = '';
    ...
    renderizarChart(series) {
        if (!this.chartInstance) {
            this.chartInstance = echarts.init(document.querySelector("#chart-governador"));
        }
