const AvaliacaoApp = {
    ...
    renderizarChart(series) {
        ...
        const options = {
            ...
            // Legenda nativa desligada: usamos a legenda em HTML acima (#legenda-avaliacao)
            legend: { show: false },
            tooltip: {
                enabled: true,
                shared: false,
                ...
                custom: function (opts) {
                    try {
                        ...
                        const badgeText = isMediaMovel ? 'Média Móvel' : (dataPoint.instituto || 'Média Móvel');
        ...
        this.chartInstance = new ApexCharts(document.querySelector("#chart-avaliacao"), options);
        this.chartInstance.render().then(() => {
            const inner = document.querySelector('#chart-avaliacao .apexcharts-inner');
